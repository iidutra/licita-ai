...
def run_extraction(opportunity: Opportunity) -> AISummary:
    ...
    if not chunks:
        # Fallback: evenly spaced chunks across the document.
        # Fast approach: get all chunk IDs, sample evenly, then fetch full objects.
        ...
        MAX_CHUNKS = 30
        all_ids = list(
            base_qs.order_by("document", "chunk_index")
        ...
        if all_ids:
            ...
            if total <= MAX_CHUNKS:
                ...
                selected_ids = [all_ids[int(i * step)] for i in range(MAX_CHUNKS)]
            chunks = list(
